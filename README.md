...
Download the Executable: Locate the standalone application file (AudienceIntelligence.exe).

Launch: Double-click the file. A background service terminal will spin up locally on your machine to host the application server.

Analyze: Within seconds, your default web browser will automatically open the full interface at [http://127.0.0.1:5000/](http://127.0.0.1:5000/).

Close: To shut down the app, simply close the terminal window running in the background.
...
Click the CONFIG button in the top header.

Enter your respective API credentials (e.g., YouTube Data API v3 Key, Google Gemini AI API Key, X Bearer Token, Telegram API ID & Hash).

Click SAVE KEYS. All credentials are saved securely to a local config.json file on your machine and are never transmitted elsewhere.
...
GitHub: github.com/harshsinghdixit
